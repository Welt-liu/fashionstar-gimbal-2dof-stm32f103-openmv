...
#define SERVO_DEAD_BLOCK 0.5 // 死区
...
#define PITCH1 60.0
#define PITCH1_SERVO_ANGLE 60.0
#define PITCH2 90.0
#define PITCH2_SERVO_ANGLE 90.0
...
extern float K_yaw2srv; // 偏航角角度到舵机角度的转换系数
extern float b_yaw2srv; // 偏航角角度到舵机角度的偏移量
// yaw = K_srv2yaw * servo + b_srv2yaw 
extern float K_srv2yaw; // 舵机角度到偏航角角度的转换系数
extern float b_srv2yaw; // 舵机角度到偏航角角度的偏移量
// 俯仰角角度转换 (pitch)
// servo = K_pitch2srv * pitch + b_pitch2srv
extern float K_pitch2srv; // 俯仰角角度到舵机角度的转换系数
extern float b_pitch2srv; // 俯仰角角度到舵机角度的偏移量
// pitch = K_srv2pitch * servo + b_srv2pitch
extern float K_srv2pitch; // 舵机角度到俯仰角角度的转换系数
extern float b_srv2pitch; // 舵机角度到俯仰角角度的偏移量 
...
extern float curYaw; // 当前偏航角角度 (单位 度)
extern float curPitch; // 当前俯仰角角度 (单位 度)
extern float nextYaw; // 下一个偏航角角度 (单位 度)
extern float nextPitch; // 下一个俯仰角角度 (单位 度)
